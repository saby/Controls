...
const _private = {
    ...
    shouldLoadChildren(self: TreeControl, nodeKey): boolean {
        ...
        const dispItem = self._listViewModel.getItemBySourceKey(nodeKey);
        const loadedChildren = dispItem && (self._options.useNewModel ?
            self._listViewModel.getChildren(dispItem, items).getCount() :
            self._listViewModel.getChildren(nodeKey, items).length);
        const isAlreadyLoaded = self.getSourceController().hasLoaded(nodeKey) || loadedChildren;
...
export class TreeControl<TOptions extends ITreeControlOptions> extends BaseControl<ITreeControlOptions> {
    private _root = null;
    ...
    protected _beforeUpdate(newOptions: TOptions) {
        ...
        if (newOptions.expandedItems && !isEqual(newOptions.expandedItems, this._listViewModel.getExpandedItems()) && newOptions.source) {
            if ((newOptions.source === this._options.source || newOptions.sourceController) && isEqual(newOptions.filter, this._options.filter) ||
                ...
                this._listViewModel.setExpandedItems(newOptions.expandedItems);
            } else {
        ...
        if (newOptions.collapsedItems && !isEqual(newOptions.collapsedItems, this._listViewModel.getCollapsedItems())) {
            this._listViewModel.setCollapsedItems(newOptions.collapsedItems);
        }
        if (this._options.markedKey !== newOptions.markedKey) {
            if (newOptions.markerMoveMode === 'leaves') {
                this._applyMarkedLeaf(newOptions.markedKey, this._listViewModel, this.getMarkerController());
            }
        }
        if (newOptions.nodeFooterTemplate !== this._options.nodeFooterTemplate) {
            this._listViewModel.setNodeFooterTemplate(newOptions.nodeFooterTemplate);
        }
        // TODO: Удалить #rea_1179794968
        if (newOptions.expanderDisplayMode !== this._options.expanderDisplayMode) {
            this._listViewModel.setExpanderDisplayMode(newOptions.expanderDisplayMode);
        }
        if (newOptions.expanderVisibility !== this._options.expanderVisibility) {
            this._listViewModel.setExpanderVisibility(newOptions.expanderVisibility);
        }
        if (newOptions.nodeProperty !== this._options.nodeProperty) {
            this._listViewModel.setNodeProperty(newOptions.nodeProperty);
        }
        if (newOptions.parentProperty !== this._options.parentProperty) {
            this._listViewModel.setParentProperty(newOptions.parentProperty);
            updateSourceController = true;
        }
        if (newOptions.hasChildrenProperty !== this._options.hasChildrenProperty) {
            this._listViewModel.setHasChildrenProperty(newOptions.hasChildrenProperty);
        }
        if (sourceController) {
    ...
    static _theme = ['Controls/treeGrid'];
