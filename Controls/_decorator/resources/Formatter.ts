...
export function splitValue(format: IFormat, value: string): string[] {
    ...
    // Когда приходит пустое значение, то всегда будет отображаться предупреждение в консоли
    // поэтому отображаем предупреждение только в том случае, когда приходит любое не пустое значение
    if (value) {
