...
const TRIGGER_VISIBILITY_DELAY = 101 * (detection.isIE ? 2 : 1);
...
const _private = {
    ...
    supportAttachLoadTopTriggerToNull(options): boolean {
        ...
        if (options.attachLoadTopTriggerToNull === false || detection.isMobileIOS) {
            return false;
        }
    ...
    setMarkerAfterScrolling(self, scrollTop) {
        ...
        if (self._markerController && self._children.listView) {
   ...
   _prepareGroups(newOptions, callback: Function) {
    ...
    _onItemSwipe(e: SyntheticEvent<Event>, item: CollectionItem<Model>, swipeEvent: SyntheticEvent<ISwipeEvent>): void {
        ...
        if (swipeEvent.nativeEvent.direction === 'left') {
            _private.setMarkedKey(this, key);
            _private.getItemActionsController(this).activateSwipe(key, swipeContainer?.width, swipeContainer?.height);
        }
        if (swipeEvent.nativeEvent.direction === 'right') {
            ...
            if (swipedItem) {
                ...
                if (this._options.multiSelectVisibility !== 'hidden') {
                    _private.getItemActionsController(this).activateRightSwipe(key);
                }
                _private.setMarkedKey(this, key);
            }
        }
